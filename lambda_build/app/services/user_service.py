...
def register_user(body: dict):
    ...
    try:
        ...
        logger.info(f"User registered in DB: {user_uuid}")
    ...
    if CARD_QUEUE_URL:
        try:
            ...
            logger.info(f"Card requests sent for user: {user_uuid}")
        except Exception as e:
            logger.warning(f"Failed to send card requests to SQS: {str(e)}")
    ...
    if NOTIFICATION_QUEUE_URL:
        try:
            ...
            logger.info(f"Registration notification sent for user: {user_uuid}")
        except Exception as e:
            logger.warning(f"Failed to send registration notification to SQS: {str(e)}")
        
    return {'userId': user_uuid, 'message': 'User registered successfully'}

def login_user(email, password):
    # Optimización: usando el GSI 'email-index' en lugar de un scan() completo
    try:
        ...
        db_user = items[0] if items else None
    except Exception as e:
        logger.error(f"Error querying user by email index: {str(e)}")
        # Fallback to scan if absolutely necessary in local/dev, but GSI is preferred
        response = table.scan()
